...
mariadb_location=/var/lib/mysql
web_root_dir=/var/www/default
...
vhost_install(){
	...
	cat > ${apache_location}/conf.d/default.conf <<EOF
...
vhost_add(){
    ...
    default_root="/var/www/${main_domain}"
...
EOF
    ...
    mkdir -p /var/wwwlog/${main_domain} ${website_root}

    cat > ${apache_location}/conf.d/${main_domain}.conf << EOF
<VirtualHost *:80>
    ServerAdmin ${email}
    ${php_admin_value}
    ...
    ErrorLog /var/wwwlog/${main_domain}/error.log
    CustomLog /var/wwwlog/${main_domain}/access.log combined
</VirtualHost>
EOF
...
EOF
    ...
    chown -R apache:apache /var/wwwlog/${main_domain} ${website_root}
    _info "All done"
}
...
<VirtualHost *:443>
    ...
    ErrorLog  /var/wwwlog/${main_domain}/ssl_error.log
    CustomLog  /var/wwwlog/${main_domain}/ssl_access.log combined
</VirtualHost>
EOF
}
...
add_letsencrypt(){
    ...
    /etc/httpd/acme.sh/acme.sh --issue --server letsencrypt ${letsdomain} -w ${website_root}
    if [ $? -eq 0 ]; then
...
add_buypass(){
    ...
    . /etc/httpd/acme.sh/acme.sh.env
    /etc/httpd/acme.sh/acme.sh -m ${email} --issue --server buypass ${letsdomain} -w ${website_root} --days 170
    if [ $? -eq 0 ]; then
        ssl_certificate="${apache_location}/conf/ssl/${main_domain}/fullchain.cer"
        ssl_certificate_key="${apache_location}/conf/ssl/${main_domain}/${main_domain}.key"
...
install_check_acme(){
    if [ -s "/etc/httpd/acme.sh/acme.sh" ]; then
        ...
        ./acme.sh --install --log --home /etc/httpd/acme.sh --certhome /etc/httpd/conf/ssl
        cd .. && rm -rf acmesh-*
...
vhost_list(){
    if [ $(ls ${apache_location}/conf.d/ | grep ".conf$" | grep -v "none" | grep -v "default" | wc -l) -gt 0 ]; then
        printf "%s\n" "ServerName"
        printf "%s\n" "----------"
        ls ${apache_location}/conf.d/ | grep ".conf$" | grep -v "none" | grep -v "default" | sed 's/.conf//g'
    else
...
vhost_del(){
    ...
    if ! apache_vhost_is_exist "${domain}"; then
        ...
        local dir=$(grep -w "DocumentRoot" ${apache_location}/conf.d/${domain}.conf | head -1 | awk '{print $2}')
        rm -f ${apache_location}/conf.d/${domain}.conf
...
apache_vhost_is_exist(){
    if [ -f "${apache_location}/conf.d/$1.conf" ]; then
        return 0
    else
        return 1
    fi
